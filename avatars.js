...
const avatars={
 phil:{name:'Phil',img:'avatars/phil.jpg',body:'standard',skin:'#f1c7a5',hair:'short',shirt:'#171717'},
 matt:{name:'Matt',img:'avatars/matt.jpg',body:'slim',skin:'#d99a70',hair:'short',shirt:'#eee8d8'},
 harry:{name:'Harry',img:'avatars/harry.jpg',body:'broad',skin:'#d6a57c',hair:'short',shirt:'#647333'}
};
let selectedAvatar='';
...
[...form.querySelectorAll('.field')].forEach(f=>{
 const txt=(f.querySelector('label')?.textContent||'').trim();
 if(['BODY TYPE','SKIN TONE','HAIR','TOP COLOUR'].includes(txt))f.classList.add('appearance-hidden');
});

const previewCard=document.querySelector('#characterSetup .preview-card');
const previewImg=document.createElement('img');
previewImg.id='selectedAvatarPreview';previewImg.className='selected-avatar-preview';previewImg.alt='Selected avatar';
const previewPlaceholder=document.createElement('div');
previewPlaceholder.className='avatar-preview-placeholder';
previewPlaceholder.innerHTML='<div>Choose your character<small>The selected avatar will appear here.</small></div>';
previewCard.append(previewImg,previewPlaceholder);

function choose(id){
 selectedAvatar=id;
 const a=avatars[id];
 character.avatar=id;character.body=a.body;character.skin=a.skin;character.hair=a.hair;character.shirt=a.shirt;
...
startBtn.onclick=e=>{
 if(!selectedAvatar){document.getElementById('setupError').textContent='Choose Phil, Matt or Harry before starting.';return}
 existingStart.call(startBtn,e);
};

const newBtn=document.getElementById('newGame'),existingNew=newBtn.onclick;
newBtn.onclick=e=>{
 selectedAvatar='';delete character.avatar;
 document.querySelectorAll('.avatar-card').forEach(c=>c.classList.remove('selected'));
 document.getElementById('characterName').value='';
 previewImg.removeAttribute('src');previewImg.classList.remove('show');previewPlaceholder.style.display='grid';
 existingNew.call(newBtn,e);
};

// Compact in-world sprite matched to the selected preset.
drawPlayer=function(c){
 ...
 if(id==='phil'){ctx.fillStyle='#9a5b32';ctx.fillRect(-5,-8,10,3)}
 ctx.restore();
};
})();